...
export const postScore = () => (dispatch) => {
	axios.post('http://localhost:3000/api/v1/scores').then(() =>
		dispatch({
			type: 'GET_SCORES'
		})
	);
...
export const postRounds = (loggedUser, resFromGame, playerNamesState) => (
	dispatch
) => {
	let data = {
...
export const postScores = (scores, holeId, rounds) => (dispatch) => {
	...
	allUsersScores.forEach((score) => {
		axios
			...
			.then((res) => dispatch(getScoresFromRes(res)));
	});
};

export const getScoresFromRes = (res) => {
	return {
		type: 'GET_SCORES_FROM_RES',
		payload: res
	};
};

export const getScores = (rounds) => async (dispatch) => {
	let allUserScores = {};
	await rounds.forEach((round) => {
		axios
			...
			.then((filteredScore) => {
				allUserScores[`${round.data.name}`] = filteredScore;
			});
	});
	dispatch({
		type: 'GET_SCORES',
